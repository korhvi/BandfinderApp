...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Call a function that depends on apiKey after the DOM content is fully loaded
    handleApiKey();
});
...
async function handleApiKey() {
    try {
        ...
        // Continue with operations that require apiKey
    } catch (error) {
...
async function searchHandler() {
    ...
    if (artistName !== '') { // Check if artist name is not empty
        try {
            const apiKey = await fetchApiKey(); // Fetch the API key
            await fetchArtistInfo(artistName, apiKey); // Call fetchArtistInfo function with apiKey
        } catch (error) {
...
async function fetchArtistInfo(artist, apiKey) {
...
async function fetchAndDisplayTopAlbums(artist, apiKey) {
    ...
    try {
        ...
        displayTopAlbum(albumData, apiKey); // Pass apiKey to displayTopAlbum function
    } catch (error) {
        console.error('Error fetching top albums:', error); // Log error if fetching fails
    }
}
...
async function fetchAndDisplaySimilarArtists(artist, apiKey) {
    ...
    try {
        ...
        displaySimilarArtists(similarArtistsData, apiKey); // Pass apiKey to displaySimilarArtists function
    } catch (error) {
        console.error('Error fetching similar artists:', error); // Log error if fetching fails
    }
}
...
function displayArtistInfo(data) {
    ...
    if (data.artist) { // If artist information is available
        ...
        resultsDiv.innerHTML = `<h2>${artistName}</h2><p>${artistSummary}</p>`;
    } else {
...
function displayTopAlbum(albumData, apiKey) {
    const resultsDiv = document.getElementById('results'); // Get reference to results div
    if (albumData.topalbums && albumData.topalbums.album.length > 0) { // If top albums are available
        ...
        resultsDiv.appendChild(albumDiv); // Append album div to results div

        displayAlbumTracks(artistName, albumName, apiKey); // Display album tracks with apiKey

        // Add event listener to previous button for navigating to the previous album
        ...
        // Add event listener to next button for navigating to the next album
        ...
        function updateAlbum(index) {
            ...
            displayAlbumTracks(newArtistName, newAlbumName, apiKey); // Display album tracks for new album with apiKey
        }
    }
}

// Function to fetch and display album tracks
async function displayAlbumTracks(artist, albumName, apiKey) {
...
function displaySimilarArtists(similarArtistsData, apiKey) {
    const resultsDiv = document.getElementById('results'); // Get reference to results div
    if (similarArtistsData.similarartists && similarArtistsData.similarartists.artist.length > 0) { // If similar artists are available
        ...
        similarArtistsData.similarartists.artist.slice(0, 12).forEach(artist => {
            // Add onclick event to fetch more information about each artist, passing apiKey as argument
            similarArtistsHTML += `<div class="similar-artist" onclick="fetchArtistInfo('${artist.name}', '${apiKey}')">${artist.name}</div>`;
        });
